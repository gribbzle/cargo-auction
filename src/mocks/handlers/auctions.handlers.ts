...
function filterAuctions(auctions: ReturnType<typeof auctionStore.getAll>, params: AuctionListRequest) {
  ...
  if (params.current_price_from != null) {
    filtered = filtered.filter((a) => (a.data.trading.price?.current ?? 0) >= params.current_price_from!)
  }

  if (params.current_price_to != null) {
    filtered = filtered.filter((a) => (a.data.trading.price?.current ?? 0) <= params.current_price_to!)
  }
...
export const handlers = [
  ...
  http.post(`${API_BASE}/auctions/:uuid/bets`, async ({ params, request }) => {
    ...
    const minPrice = Math.min(...bets.map((b) => b.price_with_vat))
    if (price === minPrice) {
