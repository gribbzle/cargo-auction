...
export async function fetchAuctionDetail(auctionUuid: string): Promise<AuctionShowResponse> {
  return get<AuctionShowResponse>(`/auctions/${auctionUuid}`);
}

export async function fetchAuctionBets(auctionUuid: string, all = false): Promise<BetListResponse> {
  return get<BetListResponse>(`/auctions/${auctionUuid}/bets`, {
    params: { all },
  });
}

export async function setBet(auctionUuid: string, data: SetBetRequest): Promise<void> {
  return post<void>(`/auctions/${auctionUuid}/bets`, data);
}

export async function toggleFavorite(auctionUuid: string): Promise<{ is_favorite: boolean }> {
