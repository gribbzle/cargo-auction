...
This document outlines the primary user journeys and identifies critical edge cases to ensure a robust and seamless user experience.
...
*   **Optimistic UI:** For critical actions (like bidding), consider using `onMutate` in TanStack Query to update the UI *before* the server responds, providing an "instant" feel (only if business logic allows).
*   **Loading States:** Use Skeletons for initial page loads and subtle progress bars/spinners for mutations.
*   **Feedback Loop:** Every significant user action (Submit, Delete, Filter) must have a visual confirmation (Success/Error Toast) or a clear change in UI state.
